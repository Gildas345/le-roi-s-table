...
const AdvancedStats = () => {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      const { data: sales, error: salesError } = await supabase
        .rpc('get_sales_stats', { days_count: period });
      ...
      const { data: products, error: productsError } = await supabase
        .rpc('get_top_products', { limit_count: 6 });
  ...
  const paidOrders = salesData.reduce((sum, day) => sum + Number(day.paid_orders || 0), 0);
  const pendingOrders = salesData.reduce((sum, day) => sum + Number(day.pending_orders || 0), 0);

  // Format data for charts
  const formattedSalesData = salesData.map(day => ({
    date: new Date(day.date).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' }),
    Commandes: Number(day.total_orders),
    'Revenus (k FCFA)': Number(day.total_revenue) / 1000,
  })).reverse();

  const pieData = [
    { name: 'Payées', value: Number(paidOrders) },
    { name: 'En attente', value: Number(pendingOrders) },
  ];
      ...
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={topProducts}>
            <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
            <XAxis dataKey="product_name" tick={{ fontSize: 11 }} angle={-45} textAnchor="end" height={80} />
            <YAxis tick={{ fontSize: 12 }} />
            <Tooltip
              contentStyle={{
                backgroundColor: 'white',
            ...
            />
            <Legend />
            <Bar dataKey="total_quantity" fill="#D4AF37" name="Quantité vendue" />
          </BarChart>
        </ResponsiveContainer>
            ...
            <div key={idx} className="flex items-center justify-between rounded-lg bg-muted/50 p-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{product.total_quantity} vendus</p>
                </div>
              </div>
              <p className="font-semibold text-accent">{Number(product.total_revenue).toLocaleString('fr-FR')} FCFA</p>
            </div>
          ))}
        </div>
      </div>
